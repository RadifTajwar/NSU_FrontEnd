...
async function continueConversation(prpt) {
    ...
    const prewrittenPrompts = [
      ...
      ` Create a story for my children's storybook about ${prpt}. Always ensure the audience is amused by the story. The climax should be the most exciting part. Avoid conversations between characters.note that the story should be short written 7 lines it is important and use longer lines..You will use the places which are located in Bangladesh as almost all the audience will be from Bangladesh so that they become thrilled while reading this story book`,
...
exports.storyLines = catchAsyncError(async (req, res, next) => {
  ...
  const filePath = path.join(__dirname, `../../LatestFrontend/public/story/user/${title}/lines/${id}.txt`);
